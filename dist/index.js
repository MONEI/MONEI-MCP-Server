...
import { getAuthorizationUrl, exchangeCodeForTokens, revokeTokens, OAuthError, } from "./auth/oauth.js";
import { getCorsOptions, httpsEnforcement, inputGuard, requireApiKeyInDev, } from "./middleware/security.js";
// ─── Configuration ──────────────────────────────────────────
const config = {
    port: parseInt(process.env.PORT ?? "3000", 10),
    host: process.env.HOST ?? "0.0.0.0",
    moneiApiBaseUrl: process.env.MONEI_API_BASE_URL ?? "https://api.monei.com/v1",
    oauth: {
        clientId: process.env.MONEI_CLIENT_ID ?? "",
        clientSecret: process.env.MONEI_CLIENT_SECRET ?? "",
        redirectUri: process.env.MONEI_REDIRECT_URI ??
...
app.get("/oauth/callback", async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            message: "MONEI account connected successfully! You can now use AI assistants with your MONEI account.",
