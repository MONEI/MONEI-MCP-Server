...
export async function handleToolCall(toolName, args, client) {
    ...
    try {
        let result;
        if (PAYMENT_TOOLS.has(toolName))
            result = await handlePaymentTool(toolName, args, client);
        else if (SUBSCRIPTION_TOOLS.has(toolName))
            result = await handleSubscriptionTool(toolName, args, client);
        else if (ACCOUNT_TOOLS.has(toolName))
            result = await handleAccountTool(toolName, args, client);
        else
            return { content: [{ type: "text", text: `Unknown tool: ${toolName}. Available: ${ALL_TOOL_DEFINITIONS.map(t => t.name).join(", ")}` }], isError: true };
