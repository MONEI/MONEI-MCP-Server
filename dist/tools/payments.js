...
export async function handlePaymentTool(toolName, args, client) {
    switch (toolName) {
        case "generate_payment_link": {
            const input = { amount: args.amount, currency: args.currency || "EUR" };
            if (args.orderId)
                input.orderId = args.orderId;
            if (args.description)
        ...
        case "get_payment": return client.getCharge(args.paymentId);
        case "list_payments": return client.listCharges({ search: args.search, status: args.status, size: args.limit || 20, from: args.offset || 0 });
        case "get_payments_kpi": return client.getChargesKPI({ start: args.start, end: args.end, interval: args.interval, timezone: args.timezone, currency: args.currency });
        case "send_payment_link": return client.sendPaymentLink({ chargeId: args.paymentId, customerEmail: args.customerEmail, customerPhone: args.customerPhone });
